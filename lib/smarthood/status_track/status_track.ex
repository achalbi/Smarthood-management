defmodule Smarthood.StatusTrack do
  @moduledoc """
  ...
  alias Smarthood.StatusTrack.KeyResultArea
  alias Smarthood.StatusTrack.Task
  @doc """
  ...
  def create_work_status(attrs \\ %{}) do
    attrs = case attrs do
        %{"notes" => notes, "status_type" => status_type, "user_id" => user_id} ->
            ...
            task_summary = Map.get(Smarthood.StatusTrack.WorkStatus.working_at_map, status_type) <> "\n" <> notes <> "\n"
  ...
  """
	...
	def update_work_status_from_tasks(%WorkStatus{} = work_status, date, user_id, attrs \\ %{}) do
        ...
        work_status_details = Map.get(Smarthood.StatusTrack.WorkStatus.working_at_map, work_status.status_type) <> "\n" <> (work_status.notes || "") <> "\n\n"
	...
	def update_work_status_with_tasks(%WorkStatus{} = work_status, attrs \\ %{}) do
            ...
            work_status_details = Map.get(Smarthood.StatusTrack.WorkStatus.working_at_map, attrs["status_type"]) <> "\n" <>  attrs["notes"] <> "\n\n"
			attrs = Map.put(attrs, "task_summary", work_status_details <> task_summary)	
		#	attrs = Map.put(attrs, "notes", work_status.notes)	
			update_work_status(work_status, attrs)
    end
	...
	def sync_with_spreadsheet(%WorkStatus{} = work_status) do
		...
		spreadsheet = Smarthood.Spreadsheets.list_spreadsheets(work_status.organization_id)
